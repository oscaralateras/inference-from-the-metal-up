...
| [T10](topics/t10_os_virtual_memory) | OS & virtual memory | **Bands filed, not yet measured.** Cold start decomposed into four series stages — NVMe → page cache → process memory → pinned → HBM — each scored against a ceiling the same box reached. The registered claim is that `mmap`'s huge apparent advantage (**≥ 3×** on the load call) **collapses to ≤ 1.2× once its deferred page faults are charged to the first pass that touches them** — on a *cold* cache. A rehearsal already showed it survives at 2.18× when *warm*, by a different mechanism: a mapping of cached pages never copies at all. | Every earlier topic starts with the weights in HBM; this is the only one about getting them there, and the only one that touches the box's slowest link — PCIe at ~25 GB/s against T7's **1,736.7 GB/s** of HBM, ~70× narrower, crossed once per model. Quoted in this repo's currency: at T6's 11.05 ms step, a 6.45 s cold start is **584 tokens not generated**, which is the scale-to-zero argument in one number. |
| [T11](topics/t11_compiler_runtime) | Compiler / runtime layer | **Bands filed, not yet measured.** T6 measured CUDA graphs on a vLLM step and got 15–36% — one number for *two* mechanisms. T11 unbundles them with a 2×2 (fusion on/off × capture on/off) over a weightless residual chain: **fusion removes bytes, capture removes launches**, and they pay off in opposite regimes. Registered: fusion is **≤ 1.1×** at batch 1 (the chain is 41 ns of traffic against a microsecond launch), capture is **≥ 2×**, and the two **cross over at batch 173** — derived from T7's measured bandwidth, not fitted. | The right compiler optimisation depends on your batch size, and the model says where: capture for low-batch latency serving, fuse for high-batch throughput, and they do not substitute for each other. The crossover shifts as `1/(2k−3)` in chain length — a 7× prediction the control run can falsify. Closes the launch-cost thread T9 opened when it measured a *collective's* fixed cost at ~34 µs. |

## Roadmap

Eleven topics, built in order. Nine shipped; T10 and T11 have their harnesses written,
tested and rehearsed, with bands pre-registered — awaiting one shared single-GPU session.
...
| T10 | OS & virtual memory | Python · GPU | 🟡 pre-registered, awaiting the pod |
| T11 | Compiler / runtime layer | Python · GPU | 🟡 pre-registered, awaiting the pod |

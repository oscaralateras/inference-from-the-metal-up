...
def quantise_symmetric(
    ...
    Args:
        ...
        granularity: how manhy scales to use"
    ...
    """
    #  Biggest integer the signed grid holds, eg int8 -> 2^7 -1 = 127.
    qmax = 2 ** (n_bits - 1) - 1
    ...
    if granularity == "per_tensor":
        ...
        # Split each row into groups and take max|w| per group -> (rows,_groups), then
        # repeat each group's value across its own columns -> back to (row, cols)
        grouped = np.abs(w).reshape(rows, n_groups, group_size).max(axis=2)
        max_abs = np.repeat(grouped, group_size, axis=1)
    else:
        raise ValueError(f"unknown granularity: {granularity!r}")

    # scale = max|w| / qmax. Clamp max_abs off zero to avoid duvide by zero on an all zero

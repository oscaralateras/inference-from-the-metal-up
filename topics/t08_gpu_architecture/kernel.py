...
**Instruction count is the budget here, not bandwidth.** A kernel this far left of the ridge has
nothing to do but wait on memory, so what decides whether it reaches the roof is how much work it
does per byte delivered — and int4 already costs roughly four times bf16's arithmetic per byte,
because one byte carries two weights instead of half of one. Every avoidable operation in the inner
loop is therefore worth removing; the scale factoring below is the largest of them.
"""
...
#
# BLOCK_N reaches down to 8 because `probe_ceiling` finds this access pattern prefers narrow blocks:
